...
public class TimetableServiceImpl implements TimetableService {
	...
	public TimetableServiceImpl(TimetableDAO timetableDAO, FacultySubjectAllocationDAO allocationDAO,
			ClassroomDAO classroomDAO, TimeSlotDAO timeSlotDAO, SectionDAO sectionDAO) {
	...
	public TimetableResponse saveTimetable(TimetableRequest request) {
		...
		FacultySubjectAllocation allocation = allocationDAO.findById(request.getAllocationId());

		if (allocation == null) {
			throw new ResourceNotFoundException("Allocation not found with ID : " + request.getAllocationId());
		}

		Classroom classroom = classroomDAO.findById(request.getClassroomId());

		if (classroom == null) {
			throw new ResourceNotFoundException("Classroom not found with ID : " + request.getClassroomId());
		}

		TimeSlot timeSlot = timeSlotDAO.findById(request.getTimeSlotId());

		if (timeSlot == null) {
			throw new ResourceNotFoundException("Time Slot not found with ID : " + request.getTimeSlotId());
		}

		Faculty faculty = allocation.getFaculty();
		Section section = allocation.getSection();

		if (timetableDAO.existsFacultyClash(faculty, request.getDayOfWeek(), timeSlot)) {

			throw new IllegalArgumentException("Faculty already has another class during this time slot.");
		}

		if (timetableDAO.existsClassroomClash(classroom, request.getDayOfWeek(), timeSlot)) {

			throw new IllegalArgumentException("Classroom is already occupied during this time slot.");
		}

		if (timetableDAO.existsSectionClash(section, request.getDayOfWeek(), timeSlot)) {

			throw new IllegalArgumentException("Section already has another class during this time slot.");
		}
	...
	public TimetableResponse updateTimetable(Long timetableId, TimetableRequest request) {
		...
		if (timetable == null) {
			throw new ResourceNotFoundException("Timetable not found with ID : " + timetableId);
		}
	...
	public List<TimetableResponse> getAllTimetables() {
		...
		return timetableDAO.findAll().stream().map(TimetableMapper::toResponse).collect(Collectors.toList());
	}
	...
	@Override
	@Transactional(readOnly = true)
	public List<TimetableResponse> getTimetablesByDay(String dayOfWeek) {

		return timetableDAO.findByDay(dayOfWeek).stream().map(TimetableMapper::toResponse).collect(Collectors.toList());
	}
	...
	@Override
	@Transactional(readOnly = true)
	public List<TimetableResponse> getTimetablesByClassroom(Long classroomId) {
		...
		return timetableDAO.findByClassroom(classroom).stream().map(TimetableMapper::toResponse)
	...
	@Override
	@Transactional(readOnly = true)
	public List<TimetableResponse> getTimetablesByFaculty(Long facultyId) {

		FacultySubjectAllocation allocation = allocationDAO.findById(facultyId);
		...
		return timetableDAO.findByFaculty(faculty).stream().map(TimetableMapper::toResponse)
				.collect(Collectors.toList());
	}
	...
	@Override
	@Transactional(readOnly = true)
	public List<TimetableResponse> getTimetablesBySection(Long sectionId) {
		...
		return timetableDAO.findBySection(section).stream().map(TimetableMapper::toResponse)
				.collect(Collectors.toList());
	}
	...
	@Override
	@Transactional(readOnly = true)
	public List<TimetableResponse> getTimetablesByAllocation(Long allocationId) {
		...
		return timetableDAO.findByAllocation(allocation).stream().map(TimetableMapper::toResponse)
				.collect(Collectors.toList());
	}
}